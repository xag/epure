...
ITEMS: list[dict] = [
    ...
    # §4.2 postconditions
    weakened("InsertPost", HUGHES, _P,
             "find k' (insert k v t) === if k == k' then Just v else find k' t",
             "the-effect-is-shown",
             "conduct/effect checks that what the write carried SHOWS (a structural subset) "
             "after the act, not that the read returns the value v; the else-branch is "
             "the-frame-holds, held by doors rather than by reading every other key back",
             "§4.2"),
    weakened("InsertPost-others", HUGHES, _P,
             "k /= k' ==> find k' (insert k v t) === find k' t",
             "the-frame-holds",
             "conduct/frame holds the boundary by DOORS (no write through a door the act "
             "did not admit), not by reading every other variable back and comparing its "
             "value — a write through an admitted door to the wrong key passes", "§4.2"),
    weakened("InsertPost-value", HUGHES, _P,
             ...
             "conduct/faithful checks that each `from` argument APPEARS in what was written, "
             "not that what the read returns equals the argument", "§4.2"),
    weakened("InsertPostSameKey", HUGHES, _P, "find k (insert k v t) === Just v",
             "the-effect-is-shown", "as InsertPost: presence of the written value, not "
             "equality of the read with it", "§4.2"),
    weakened("DeletePost", HUGHES, _P,
             "find k' (delete k t) === if k == k' then Nothing else find k' t",
             "the-effect-is-shown",
             "the then-branch is conduct/effect's deletes check (the read no longer shows "
             "it); the else-branch is the-frame-holds, by doors", "§4.2"),
    weakened("DeletePost-others", HUGHES, _P,
             "k /= k' ==> find k' (delete k t) === find k' t", "the-frame-holds",
             "as InsertPost-others", "§4.2"),
    weakened("FindPostPresent", HUGHES, _P, "find k (insert k v t) === Just v",
             "the-effect-is-shown", "as InsertPostSameKey", "§4.2"),
    ...
    owed("DeleteInsertWeak", HUGHES, _M,
         "k /= k' ==> delete k (insert k' v' t) ≏ insert k' v' (delete k t)",
         "independent-writes-commute", TWO_STRETCHES, "Appendix A"),
    owed("DeleteInsert", HUGHES, _M,
         "delete k (insert k' v' t) ≏ if k == k' then delete k t "
         "else insert k' v' (delete k t)",
         "undo-restores", TWO_STRETCHES, "Appendix A"),
    owed("DeleteDelete", HUGHES, _M, "delete k (delete k' t) ≏ delete k' (delete k t)",
         "independent-writes-commute", TWO_STRETCHES, "Appendix A"),
    ...
    owed("UnionDeleteInsert", HUGHES, _M,
         ...
         "a-merge-keeps-both-and-prefers-the-left", MERGE, "Appendix A"),
    owed("UnionUnionIdem", HUGHES, _M, "union t t ≏ t", "twice-is-once", TWO_STRETCHES,
         "Appendix A"),
    ...
    weakened("FindInsert", HUGHES, _M,
             "find k (insert k' v' t) === if k == k' then Just v' else find k t",
             "the-effect-is-shown", "as InsertPost", "Appendix A"),
    owed("FindInsert-later", HUGHES, _M,
         "find k t, read again after later acts that do not name k, === find k t",
         "shown-once-shown-until-touched", TWO_STRETCHES,
         "the else-branch of FindInsert read over time, Appendix A"),
    weakened("FindDelete", HUGHES, _M,
             "find k (delete k' t) === if k == k' then Nothing else find k t",
             "the-effect-is-shown", "as DeletePost", "Appendix A"),
    owed("FindUnion", HUGHES, _M, "find k (union t t') === (find k t <|> find k t')",
         "a-merge-keeps-both-and-prefers-the-left", MERGE, "Appendix A"),

    # §4.3 preservation of equivalence
    owed("InsertPreservesEquivWeak", HUGHES, _E,
         "t ≏ t' ==> insert k v t ≏ insert k v t'", "equivalent-worlds-stay-equivalent",
         ABSTRACTION, "§4.3"),
    owed("InsertPreservesEquiv", HUGHES, _E, "insert k v t ≏ insert k v t' (t ≏ t' generated)",
         "equivalent-worlds-stay-equivalent", ABSTRACTION, "§4.3, Fig. 5"),
    owed("DeletePreservesEquiv", HUGHES, _E, "delete k t ≏ delete k t'",
         "equivalent-worlds-stay-equivalent", ABSTRACTION, "Fig. 5"),
    owed("UnionPreservesEquiv", HUGHES, _E, "union t1 t2 ≏ union t1' t2'",
         "a-merge-keeps-both-and-prefers-the-left", MERGE, "Fig. 5"),
    owed("FindPreservesEquiv", HUGHES, _E, "find k t === find k t'",
         "same-state-same-story", ABSTRACTION, "Fig. 5"),
    ...
    owed("InsertComplete", HUGHES, _I, "t === foldl (flip (uncurry insert)) nil (insertions t)",
         "every-world-is-constructible", ABSTRACTION, "§4.4"),
    owed("InsertCompleteForDelete", HUGHES, _I, "InsertComplete (delete k t)",
         "every-world-is-constructible", ABSTRACTION, "§4.4"),
    ...
    owed("NilModel", HUGHES, _B, "toList nil === []", "the-world-agrees-with-the-model",
         ABSTRACTION, "§4.5, Fig. 6"),
    owed("InsertModel", HUGHES, _B,
         "toList (insert k v t) === L.insert (k, v) (deleteKey k (toList t))",
         "the-world-agrees-with-the-model", ABSTRACTION, "§4.5, Fig. 6"),
    owed("DeleteModel", HUGHES, _B, "toList (delete k t) === deleteKey k (toList t)",
         "the-world-agrees-with-the-model", ABSTRACTION, "§4.5, Fig. 6"),
    ...
    owed("FindModel", HUGHES, _B, "find k t === L.lookup k (toList t)",
         "the-world-agrees-with-the-model", ABSTRACTION, "§4.5, Fig. 6"),
    ...
    covered("safe", RFC, "method property",
            ...
            "conduct/frame on an act declaring touches.via = []: any write through a door the "
            "model knows convicts", "§9.2.1"),
    owed("idempotent", RFC, "method property",
         "the intended effect on the server of multiple identical requests with that method is "
         "the same as the effect for a single such request", "twice-is-once", TWO_STRETCHES,
         "§9.2.2"),
