...
| `epure.draft` | proposes, from the same projected worlds `agrees` judges, each action's updates (frame, constant, argument, variable, increment, saturating increment — simplest first) and the booleans its pre-worlds support as a guard, then holds every proposal equivalent to the hand-written expression over the whole finite domain — a measurement of how much of the arithmetic the tapes determine (the ledger's `the-tapes-draft-the-arithmetic`), never a generator that installs: a guard is drafted from refusals or not at all, and a constant that fits two samples of one day is counted different from the argument the person meant |
