...
def test_every_law_is_checked_or_owed_and_the_owed_are_these():
    ...
    for law in CONDUCT_LAWS:
        ...
        for n in (native.split(", ") if native else []):
            assert n in declared | {"model/prove", "model/promised"}, \
                f"{law.id} names {n}, which is not declared"
    ...
    assert {n for law in CONDUCT_LAWS for n in (law.payload.get("native") or "").split(", ")
            if n} == declared | {"model/prove", "model/promised"}
...
def test_the_catalogue_is_twenty_two_laws_and_the_census_says_how_many_it_holds():
    ...
    assert len(CONDUCT_LAWS) == 22
    assert len({law.id for law in CONDUCT_LAWS}) == 22
